...
class PrettyObject(PrettyRepr):
    ...
    def __pretty_repr__(self) -> Generator[Union[PrettyType, PrettyAttr], None, None]:
        ...
        Union[PrettyType, PrettyAttr]
            Pretty representation items generated by
    ...
    def __pretty_repr_item__(self, k: Any, v: Any) -> Optional[Tuple[Any, Any]]:
        ...
        Optional[Tuple[Any, Any]]
            A tuple containing the (key, value), or None
...
def flat_mapping(
    ...
    *,
    ...
    ----------
    xs
        A nested mapping to flatten.
    keep_empty_nodes
        If True, replaces empty mappings with ``empty_node`` sentinel.
        Otherwise, empty mappings are omitted from the result.
    is_leaf
        Optional function that takes ``(path, mapping)`` and returns True if
        the mapping should be treated as a leaf (i.e., not flattened further).
        Defaults to treating all mappings as non-leaves.
    sep
        If specified, keys in the returned mapping will be ``sep``-joined strings
...
def nest_mapping(
    ...
    *,
    ...
    ----------
    xs
        A flattened mapping with tuple or string keys.
    sep
        Separator used to split string keys into paths. Must match the separator
...
def _default_compare(x: Any, values: set) -> bool:
    ...
    x
        The object to check.
    values
        A set of object identities to compare against.

    Returns
    -------
    bool
        True if the object's id is in the values set.
    """
...
class PrettyDict(dict, PrettyRepr):
    ...
    def __getattr__(self, key: K):  # type: ignore[misc]
        ...
        ------
        AttributeError
            If a dunder (``__x__``) attribute is missing. The
            ...
            on a ``KeyError``.
        KeyError
            If a non-dunder key is not found in the dictionary.
        """
    ...
    def __repr__(self) -> str:
        ...
        -------
        str
            A formatted string representation using pretty printing.
        """
    ...
    def split(self, *filters: Filter) -> Union['PrettyDict', Tuple['PrettyDict', ...]]:
        ...
        ----------
        *filters
            Filter specifications to split the dictionary.
    ...
    def filter(self, *filters: Filter) -> Union['PrettyDict', Tuple['PrettyDict', ...]]:
        ...
        ----------
        *filters
            Filter specifications to apply.
    ...
    def merge(self, *states: 'PrettyDict') -> 'PrettyDict':
        ...
        ----------
        *states
            Additional PrettyDict objects to merge.
    ...
    def subset(self, *filters: Filter) -> Union['PrettyDict', Tuple['PrettyDict', ...]]:
        ...
        ----------
        *filters
            Filter specifications for subsetting.

        Returns
        -------
        Union[PrettyDict, Tuple[PrettyDict, ...]]
            One or more subsetted
            dictionaries.
        """
...
class NestedStateRepr(PrettyRepr):
    ...
    ----------
    state
        The PrettyDict to wrap for pretty representation.

    Attributes
    ----------
    state : PrettyDict
        The wrapped dictionary state.
    """
    ...
    def __pretty_repr__(self) -> Generator[Union[PrettyType, PrettyAttr], None, None]:
        ...
        ------
        Union[PrettyType, PrettyAttr]
            Pretty representation items, skipping the
    ...
    def __treescope_repr__(self, path: Any, subtree_renderer: Callable) -> Any:
        ...
        subtree_renderer
            Callable to render subtrees.

        Returns
        -------
        Any
            Rendered representation of the nested state.
        """
        children = {}
...
def _default_repr_object(node: PrettyDict) -> Generator[PrettyType, None, None]:
    ...
    ----------
    node
        The PrettyDict node to represent.

    Yields
    ------
    PrettyType
        A type representation with dict-like formatting.
    """
    yield PrettyType('', value_sep=': ', start='{', end='}')
...
def _default_repr_attr(node: PrettyDict) -> Generator[PrettyAttr, None, None]:
    ...
    ----------
    node
        The PrettyDict node whose attributes to represent.

    Yields
    ------
    PrettyAttr
        Pretty attribute representations for each item.
    """
...
class NestedDict(PrettyDict):
    ...
    See Also
    --------
    FlattedDict : The flattened counterpart with tuple keys.
    flat_mapping : Function to flatten a nested mapping.
    nest_mapping : Function to unflatten a flat mapping.
    """
    __module__ = 'brainstate.util'
    ...
    def split(self, *filters: Filter) -> Union['NestedDict', Tuple['NestedDict', ...]]:
        ...
        first
            The first filter
        *filters
            The optional, additional filters to group the state into mutually exclusive substates.
    ...
    def merge(*states) -> 'NestedDict':
        ...
        ----------
        *states
            Additional :class:`PrettyDict` objects.
    ...
    def replace_by_pure_dict(
        ...
        ----------
        pure_dict
            A pure dictionary with matching structure containing new values.
        replace_fn
            Optional custom function to replace values. Takes ``(old_value, new_value)``
        ...
        ------
        ValueError
            If a key in ``pure_dict`` is not found in this NestedDict.
...
class FlattedDict(PrettyDict):
    ...
    def assign_dict_values(self, data: Dict[PathParts, Any]) -> None:
        ...
        ----------
        data
            A dictionary containing the values to assign, where keys must match
            those in the FlattedDict.

        Raises
        ------
        KeyError
            If a key in the FlattedDict is not present in the provided dictionary.
...
def _split_nested_mapping(
    ...
    *filters: Filter,
    ...
    ----------
    mapping
        The NestedDict to split.
    *filters
        Filter specifications. The catch-all filters ``...`` or ``True`` can
        only be used as the last filter.

    Returns
    -------
    Tuple[NestedDict, ...]
        A tuple of n+1 NestedDicts, where n is the number
        of filters. The last mapping contains items that didn't match any filter.

    Raises
    ------
    ValueError
        If ``...`` or ``True`` is used before the last filter position.
    AssertionError
        If mapping is not a NestedDict.
    """
...
def _split_flatted_mapping(
    ...
    *filters
    ...
    -------
    Tuple[FlattedDict, ...]
        A tuple of n+1 FlattedDicts, where n is the number
        of filters. The last mapping contains items that didn't match any filter.
    ...
    ValueError
        If ``...`` or ``True`` is used before the last filter position.
    AssertionError
        If mapping is not a FlattedDict.
    """
    # Check if the filters are exhaustive
    for i, filter_ in enumerate(filters):
        if filter_ in (..., True) and i != len(filters) - 1:
...
def _nest_unflatten(
    ...
    ----------
    static
        Tuple of keys for reconstruction.
    leaves
        Tuple of leaf values.

    Returns
    -------
    NestedDict
        Reconstructed NestedDict.
    """
...
class PrettyList(list, PrettyRepr):
    ...
    def tree_unflatten(cls, aux_data: Tuple, children: list) -> 'PrettyList':
        ...
        children
            List items to reconstruct from.
...
def _list_repr_attr(node: PrettyList) -> Generator[PrettyAttr, None, None]:
    ...
    ----------
    node
        The PrettyList whose items to represent.

    Yields
    ------
    PrettyAttr
        Pretty attribute representations for each item with empty keys.
    """
    for v in node:
        if isinstance(v, list):
            v = PrettyList(v)
...
def _list_repr_object(node: PrettyList) -> Generator[PrettyType, None, None]:
    ...
    ----------
    node
        The PrettyList to represent.

    Yields
    ------
    PrettyType
        A type representation with list-like formatting.
    """
    yield PrettyType('', value_sep='', start='[', end=']')
...
def _repr_object_general(node: Any) -> Generator[PrettyType, None, None]:
    ...
    ------
    PrettyType
        A PrettyType object representing the type of the node,
...
def _repr_attribute_general(node: Any) -> Generator[PrettyAttr, None, None]:
    ...
    ------
    PrettyAttr
        A PrettyAttr object representing the key and value of

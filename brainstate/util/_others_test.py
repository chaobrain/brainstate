...
class TestClearBufferMemory(unittest.TestCase):
    """Test cases for clear_buffer_memory function."""

    @patch('brainstate.util.others.xla_bridge')
    @patch('brainstate.util.others.gc')
    @patch('brainstate.util.others.jax')
    def test_clear_arrays_only(self, mock_jax, mock_gc, mock_xla):
        """Test clearing array buffers only."""
        # Setup mocks
        mock_buffer = MagicMock()
        mock_backend = MagicMock()
        mock_backend.live_buffers.return_value = [mock_buffer]
        mock_xla.get_backend.return_value = mock_backend

        # Call function
        clear_buffer_memory(array=True, compilation=False)

        # Verify calls
        mock_xla.get_backend.assert_called_once_with(None)
        mock_backend.live_buffers.assert_called_once()
        mock_buffer.delete.assert_called_once()
        mock_jax.clear_caches.assert_not_called()
        mock_gc.collect.assert_called_once()

    @patch('brainstate.util.others.xla_bridge')
    @patch('brainstate.util.others.gc')
    @patch('brainstate.util.others.jax')
    def test_clear_compilation_only(self, mock_jax, mock_gc, mock_xla):
        """Test clearing compilation cache only."""
        clear_buffer_memory(array=False, compilation=True)

        mock_xla.get_backend.assert_not_called()
    ...
    @patch('brainstate.util.others.gc')
    @patch('brainstate.util.others.jax')
    def test_clear_both(self, mock_jax, mock_gc, mock_xla):
        """Test clearing both arrays and compilation cache."""
        mock_buffer = MagicMock()
        mock_backend = MagicMock()
        mock_backend.live_buffers.return_value = [mock_buffer]
        mock_xla.get_backend.return_value = mock_backend

        clear_buffer_memory(array=True, compilation=True)

        mock_xla.get_backend.assert_called_once()
        mock_buffer.delete.assert_called_once()
        mock_jax.clear_caches.assert_called_once()
        mock_gc.collect.assert_called_once()
    ...
    @patch('brainstate.util.others.xla_bridge')
    @patch('brainstate.util.others.gc')
    @patch('brainstate.util.others.warnings')
    def test_error_handling(self, mock_warnings, mock_gc, mock_xla):
        """Test error handling during buffer clearing."""
        mock_xla.get_backend.side_effect = Exception("Test error")

        clear_buffer_memory(array=True)

        mock_warnings.warn.assert_called_once()
        warning_msg = mock_warnings.warn.call_args[0][0]
        self.assertIn("Failed to clear buffers", warning_msg)
        mock_gc.collect.assert_called_once()

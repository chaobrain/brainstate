...
class TestIsConnection(unittest.TestCase):
    """Test suite for _is_connection function.

    Note: These tests are skipped because _is_connection expects real JAX JaxprEqn objects
    which are difficult to mock properly. The function is tested indirectly through
    integration tests in the compiler module.
    """

    def setUp(self):
        """Set up test fixtures."""
        import brainstate._compatible_import as compat
        self.compat = compat
        self.original_is_jit = compat.is_jit_primitive

    def tearDown(self):
        """Restore original is_jit_primitive."""
        self.compat.is_jit_primitive = self.original_is_jit

    def create_mock_eqn(self, primitive_name, params=None):
        """Helper to create mock equation."""
        eqn = Mock()
        eqn.primitive = Mock()
        eqn.primitive.name = primitive_name
        eqn.params = params if params is not None else {}
        return eqn

    @unittest.skip("Requires real JAX JaxprEqn objects, tested via integration tests")
    def test_jit_with_brainevent_name_returns_true(self):
        """Test that jit primitive with 'brainevent' in name param returns True."""
        eqn = self.create_mock_eqn('jit', {'name': 'brainevent_connection'})
        self.compat.is_jit_primitive = lambda x: True

        result = _is_connection(eqn)
        self.assertTrue(result)

    @unittest.skip("Requires real JAX JaxprEqn objects, tested via integration tests")
    def test_jit_without_brainevent_name_returns_false(self):
        """Test that jit primitive without 'brainevent' returns False."""
        eqn = self.create_mock_eqn('jit', {'name': 'some_other_function'})
        self.compat.is_jit_primitive = lambda x: True
    ...
    @unittest.skip("Requires real JAX JaxprEqn objects, tested via integration tests")
    def test_jit_without_name_param_returns_false(self):
        """Test that jit primitive without name param returns False."""
        eqn = self.create_mock_eqn('jit', {})
        self.compat.is_jit_primitive = lambda x: True
    ...
    @unittest.skip("Requires real JAX JaxprEqn objects, tested via integration tests")
    def test_brainevent_primitives_return_true(self):
        """Test that brainevent connection primitives return True."""
        primitive_names = [
            'binary_fixed_num_mv',
            'event_ell_mv',
            'event_csr_matvec',
        ...
        self.compat.is_jit_primitive = lambda x: False

        for prim_name in primitive_names:
            with self.subTest(primitive=prim_name):
                eqn = self.create_mock_eqn(prim_name)
                result = _is_connection(eqn)
                self.assertTrue(result, f"Primitive '{prim_name}' should return True")

    @unittest.skip("Requires real JAX JaxprEqn objects, tested via integration tests")
    def test_dot_general_primitive_returns_true(self):
        """Test that dot_general_p primitive returns True."""
        eqn = self.create_mock_eqn('dot_general')
    ...
    @unittest.skip("Requires real JAX JaxprEqn objects, tested via integration tests")
    def test_conv_general_dilated_primitive_returns_true(self):
        """Test that conv_general_dilated_p primitive returns True."""
        eqn = self.create_mock_eqn('conv_general_dilated')
        eqn.primitive = conv_general_dilated_p
        self.compat.is_jit_primitive = lambda x: False
    ...
    @unittest.skip("Requires real JAX JaxprEqn objects, tested via integration tests")
    def test_non_connection_primitives_return_false(self):
        """Test that non-connection primitives return False."""
        primitive_names = ['add', 'mul', 'sub', 'div', 'broadcast_in_dim', 'custom_primitive']
        ...
        for prim_name in primitive_names:
            with self.subTest(primitive=prim_name):
                eqn = self.create_mock_eqn(prim_name)
                result = _is_connection(eqn)
                self.assertFalse(result, f"Primitive '{prim_name}' should return False")

...
class TestVmap(unittest.TestCase):
    def test_vmap_1(self):
        ...
        r2 = brainstate.transform.vmap(model, state_in_axes=True, axis_size=5)()
    ...
    def test_vmap_2(self):
        class Model(brainstate.nn.Module):
            def __init__(self):
                ...
                self.c = brainstate.State(brainstate.random.randn(1), tag='c')
        ...
        model = Model()
        r2 = brainstate.transform.vmap(model,
                                       state_in_axes=brainstate.ShortTermState,
                                       axis_size=5)()
    ...
    def test_vmap_3(self):
        ...
        with self.assertRaises(BatchAxisError):
            r2 = brainstate.transform.vmap(model, state_in_axes=model.states(), out_states={1: model.states()})()
    ...
    def test_vmap_with_random(self):
        ...
        model = Model()
        r2 = brainstate.transform.vmap(
            model,
            state_in_axes=model.states(brainstate.ShortTermState),
            out_states=model.c
        )(
    ...
    def test_vmap_states_and_input_1(self):
        ...
        @brainstate.transform.vmap(state_in_axes=gru.states(brainstate.HiddenState))
        def forward(x):
            return gru(x)
    ...
    def test_vmap_jit(self):
        ...
        @brainstate.transform.vmap(state_in_axes=foo.states())
        def mul():
            foo()
    ...
    def test_auto_rand_key_split(self):
        ...
        res = brainstate.transform.vmap(f, axis_size=10)()
    ...
    def test_axis(self):
        ...
        r2 = brainstate.transform.vmap(f, axis_name='i')(jnp.arange(10))
        print(r2)
    ...
    def test_vmap_init(self):
        ...
        @brainstate.transform.vmap_new_states(state_tag='new1', axis_size=5)
        def init1():
            foo.init_state_v1()
        ...
        @brainstate.transform.vmap_new_states(state_tag='new2', axis_size=5, state_in_axes=foo.states('new1'))
        def init2():
            foo.init_state_v2()
...
class TestVMAPNewStatesEdgeCases(unittest.TestCase):
    ...
    def test_axis_size_zero(self):
        ...
        with self.assertRaises(ValueError):
            @brainstate.transform.vmap_new_states(state_tag='new1', axis_size=0)
            def faulty_init():
                foo.init_state()
    ...
    def test_axis_size_negative(self):
        ...
        with self.assertRaises(ValueError):
            @brainstate.transform.vmap_new_states(state_tag='new1', axis_size=-3)
            def faulty_init():
                foo.init_state()

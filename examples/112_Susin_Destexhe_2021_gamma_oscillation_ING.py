...
class INGNet(bst.nn.DynamicsGroup):
    def __init__(self):
        ...
        self.ext_to_FS = bst.nn.DeltaProj(
            comm=bst.event.FixedProb(self.num_rs, self.num_fs, 0.02, self.ext_weight),
            post=self.fs_pop,
            label='ge'
        )
        self.ext_to_RS = bst.nn.DeltaProj(
            comm=bst.event.FixedProb(self.num_rs, self.num_rs, 0.02, self.ext_weight),
            post=self.rs_pop,
            label='ge'
        )
        self.ext_to_RS2 = bst.nn.DeltaProj(
            comm=bst.event.FixedProb(self.num_rs, self.num_fs2, 0.02, self.ext_weight),
            post=self.fs2_pop,
            label='ge'
        )
        ...
        self.RS_to_FS2 = bst.nn.DeltaProj(
            self.rs_pop.prefetch('spike').delay.at(self.delay),
            comm=bst.event.FixedProb(self.num_rs, self.num_fs2, 0.15, self.exc_syn_weight),
            post=self.fs2_pop,
            label='ge'
        )
        ...
        self.FS_to_RS = bst.nn.DeltaProj(
            ...
            comm=bst.event.FixedProb(self.num_fs, self.num_rs, 0.02, self.inh_syn_weight),
            post=self.rs_pop,
            label='gi'
        )
        self.FS_to_FS = bst.nn.DeltaProj(
            self.fs_pop.prefetch('spike').delay.at(self.delay),
            comm=bst.event.FixedProb(self.num_fs, self.num_fs, 0.02, self.inh_syn_weight),
            post=self.fs_pop,
            label='gi'
        )
        self.FS_to_FS2 = bst.nn.DeltaProj(
            self.fs_pop.prefetch('spike').delay.at(self.delay),
            comm=bst.event.FixedProb(self.num_fs, self.num_fs2, 0.03, self.inh_syn_weight),
            post=self.fs2_pop,
            label='gi'
        )
        ...
        self.FS2_to_RS = bst.nn.DeltaProj(
            self.fs_pop.prefetch('spike').delay.at(self.delay),
            comm=bst.event.FixedProb(self.num_fs2, self.num_rs, 0.15, self.exc_syn_weight),
            post=self.rs_pop,
            label='gi'
        )
        self.FS2_to_FS = bst.nn.DeltaProj(
            self.fs_pop.prefetch('spike').delay.at(self.delay),
            comm=bst.event.FixedProb(self.num_fs2, self.num_fs, 0.15, self.exc_syn_weight),
            post=self.fs_pop,
            label='gi'
        )
        self.FS2_to_FS2 = bst.nn.DeltaProj(
            self.fs_pop.prefetch('spike').delay.at(self.delay),
            comm=bst.event.FixedProb(self.num_fs2, self.num_fs2, 0.6, self.exc_syn_weight),
            post=self.fs2_pop,
            label='gi'
        )

...
with bst.environ.context(dt=1.0 * u.ms):
  ...
  @bst.compile.jit
  def train_fn(xs, ys):
    grads, loss, correct_n = bst.augment.grad(loss_fun, net.states(bst.ParamState), has_aux=True, return_value=True)(xs, ys)
  ...
  for epoch_i in range(n_epoch):
    ...
    t = (time.time() - t0) / 60
    print(f"Epoch {epoch_i}: train_loss={train_loss:.3f}, train_acc={train_acc:.3f}, "
          f"test_loss={test_loss:.3f}, test_acc={test_acc:.3f}, time={t:.2f} min")
